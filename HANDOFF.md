...
**IN PROGRESS 0.2 — partition `logs` by month (REHEARSED ON DOCKER, NOT on VPS, NOT committed).** Plan: `docs/plans/2026-09-05-xdr-phase0-foundations.md` Task 0.2.
...
3. **Drop `logs_old`** on Docker once satisfied (`DROP TABLE logs_old;`).
4. **VPS deploy (only after 1-3):** on the VPS — `GRANT CREATE ON SCHEMA public TO cybertools_app` (ops role + OPS_DB_URL already exist from 0.1); run the partitioning migration during a quiet window; **backfill 560k+ rows will be heavier than local** — consider batching the `INSERT SELECT` by month and running outside the txn for very large tables, or accept a brief lock. Verify, then `DROP TABLE logs_old`. Never `sudo pm2`.

...
- **ATT&CK COVERAGE HEAT MAP — PLAN WRITTEN (`docs/plans/2026-09-06-attack-coverage-heatmap.md`), not built.** The Phase 1.6 map it was meant to reuse never existed; this plan builds it directly on the existing `attack_techniques[]` field: one read-only `GET /api/siem/attack/coverage` endpoint (rule-coverage + alert-activity counts via UNNEST over the enabled set incl. the catalog enablement predicate) + an `AttackCoverage.jsx` matrix in the Rule Library tab (14 tactics × union technique set, shaded by count, metric toggle, click→technique filter). No migration (unless `correlation_rules` lacks `attack_techniques`). **User's decision: build this BEFORE XDR Phase 2.**

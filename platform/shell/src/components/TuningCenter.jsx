...
const s = {
  ...
  table: { width: '100%', borderCollapse: 'collapse', fontSize: '12px', tableLayout: 'fixed', fontFamily: 'var(--font-mono)' },
...
export default function TuningCenter() {
  ...
  const runAnalysis = async () => {
    ...
    try {
      const h = await authHeaders();
      const res = await fetch(`${API}/run`, { method: 'POST', headers: h, signal: AbortSignal.timeout(300000) });
      if (!res.ok) {
        setRunning(false);
